...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--env-file', type=str, help='<Required> Set flag', required=False, default=False)
    try:
        args = parser.parse_args()
        print(args)
        env = load_env_file(args.env_file) if args.env_file else load_all_env_vars()
        description = load_json_description(args.env_file)
        errors = is_valid_env(description, env)
        if errors.__len(): print(list(errors))
        print(description)
